...
contract RWAOracle {
    ...
    function submitRWAProof(
        ...
        // 1. REAL PROOF VERIFICATION (Always runs)
        ...
        // 2. Conditional Collateral Credit
        // We only increment balance if the nullifier is new.
        // This allows 'any number of verifications' as requested, while preventing inflation.
        bool isNewAttestation = !nullifiers[nullifierHash];
        if (isNewAttestation) {
            nullifiers[nullifierHash] = true;
            activeCollateral[msg.sender] += minCollateral;
        }

        // 3. Always emit event on successful verification
